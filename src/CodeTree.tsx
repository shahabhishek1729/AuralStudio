...
function getNodeFill(node) {
  if (node.data.name.includes("variable"))
    return "blue";
  if (node.data.name.includes("function"))
    return "red";
  if (node.data.name.includes("class"))
    return "green";
  else
    return "black";
}
...
function TreeChart({ data }) {
  ...
  useEffect(() => {
      ...
      const linkGen = d3.linkVertical().y(node => getY(node, maxLevels)).x(node => node.x);
      ...
      //   .data(root.descendants())
      //   .append("rect")
      //   .attr("x", function(d) { return getY(d); })
      ...
      //   .attr("fill", "white")
        ...
        // .attr("text-anchor", "middle")
        // .attr("font-size", 18)
        // .attr("fill", node => node.data.name === "function" ? "white" : "blue")
        // .attr("x", getY)
        // .attr("y", node => node.x - 19)
  }, [data, dimensions]);
